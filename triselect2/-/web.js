        ...
        function $me_atom2_ctx_rect(p) {
            ...
            if (!p.stroke ||
                ...
                if (ctxStrokeWidthTop && ctxStrokeStyleTop != 'transparent') {
                    ...
                    p.ctx.lineTo(Math.round(x += p.ctxWidth - ctxBorderRadiusLeftTop - ctxBorderRadiusRightTop - ctxStrokeWidthLeft / 2 - ctxStrokeWidthRight / 2), Math.round(y -= ctxBorderRadiusLeftTop));
                ...
                if (ctxStrokeWidthBottom && ctxStrokeStyleBottom != 'transparent') {
                    if (x == null) {
                        p.ctx.beginPath();
                        p.ctx.moveTo(Math.round(x = p.ctxLeft + p.ctxWidth - ctxStrokeWidthRight / 2), Math.round(y = p.ctxTop + p.ctxHeight - ctxBorderRadiusRightBottom - ctxStrokeWidthBottom / 2));
                    }
                    ...
                    p.ctx.lineTo(Math.round(x -= p.ctxWidth - ctxBorderRadiusLeftBottom - ctxBorderRadiusRightBottom - ctxStrokeWidthRight / 2 - ctxStrokeWidthLeft / 2), Math.round(y += ctxBorderRadiusRightBottom));
            ...
            control: {
                option: $$.$me_atom2_prop({ keys: ['.option_ids'] }, ({ key: [id] }) => ({
                    ...
                    prop: {
                            ...
                            const result = isSelected ? options[id].caption :
                                typeof options[id].captionShort == 'string' ?
                                    ...
                                    typeof options[id].captionShort == 'function' ?
                                        ...
                                        $$.$me_throw(options[id].captionShort);
                            return result;
                        }),
                        ...
                        borderWidthLeft: $$.$me_atom2_prop(['.isSelected', '.idx', '<.option_ids', '<.value', '<.borderWidth'], ({ masters: [isSelected, idx, option_ids, value, borderWidth] }) => {
                            const idx_selected = option_ids.indexOf(value);
                            return isSelected || !idx || idx == option_ids.length - 1 && !idx_selected ? borderWidth : 0;
                        }),
                        colorBorderLeft: $$.$me_atom2_prop(['.isSelected', '.idx', '<.option_ids', '<.value', '<.colorBorder', '<.colorBorderSelected'], ({ masters: [isSelected, idx, option_ids, value, colorBorder, colorBorderSelected] }) => {
                            const idx_selected = option_ids.indexOf(value);
                            return isSelected ? colorBorderSelected : !idx || idx == option_ids.length - 1 && !idx_selected ? colorBorder : 'transparent';
                        }),
                        borderWidthRight: $$.$me_atom2_prop(['.isSelected', '.idx', '<.option_ids', '<.value', '<.borderWidth'], ({ masters: [isSelected, idx, option_ids, value, borderWidth] }) => {
                            const idx_selected = option_ids.indexOf(value);
                            return isSelected || idx == option_ids.length - 1 || !idx && idx_selected == option_ids.length - 1 ? borderWidth : 0;
                        }),
                        colorBorderRight: $$.$me_atom2_prop(['.isSelected', '.idx', '<.option_ids', '<.value', '<.colorBorder', '<.colorBorderSelected'], ({ masters: [isSelected, idx, option_ids, value, colorBorder, colorBorderSelected] }) => {
                            const idx_selected = option_ids.indexOf(value);
                            return isSelected ? colorBorderSelected : idx == option_ids.length - 1 || !idx && idx_selected == option_ids.length - 1 ? colorBorder : 'transparent';
                        }),
                    ...
                    prop: {
                        ...
                        borderLeft: $$.$me_atom2_prop(['<.idx_selected', '<.option_ids', '<.colorBorder', '<.colorBorderSelected', '<.borderWidth'], ({ masters: [idx_selected, option_ids, colorBorder, colorBorderSelected, borderWidth] }) => {
                            ...
                            const result = ~idx_selected.indexOf(idx) ?
                                ...
                                !idx || !~idx_selected.indexOf(idx - 1) ? { width: borderWidth, color: colorBorder } : { width: borderWidth, color: 'transparent' };
                            return result;
                        }),
                        ...
                        borderRight: $$.$me_atom2_prop(['<.idx_selected', '<.option_ids', '<.colorBorder', '<.colorBorderSelected', '<.borderWidth'], ({ masters: [idx_selected, option_ids, colorBorder, colorBorderSelected, borderWidth] }) => {
                            ...
                            const result = ~idx_selected.indexOf(idx) ? { width: borderWidth, color: colorBorderSelected } :
                                idx == option_ids.length - 1 ? { width: borderWidth, color: colorBorder } :
...
(function ($) {
    var $$;
    (function ($$) {
        $$.$nl_triselect_demo = (rootElem) => {
            $$.$nl_app_defaults_init();
            return new $$.$me_atom2_elem({ tail: 'app', cnf: {
                    node: rootElem,
                    base: $$.$me_demo4,
                    prop: {
                        demoFor: () => '$nl_triselect',
                        defs: () => ({
                            ...
                            rightBottom: Object.assign({}, defaults, { options: () => ({
                                    ...
                                    '1': { caption: 'Кроме апартаментов', captionShort: 'Кроме' },
                                    '2': { caption: 'Только апараменты', captionShort: 'Только' },
                                }) }),
                        }),
                    },
                    style: {},
